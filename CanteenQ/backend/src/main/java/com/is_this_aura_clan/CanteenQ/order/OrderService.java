...
public class OrderService {
	...
	public OrderResponse placeOrder(FirebaseAuthenticationPrincipal principal, OrderRequest request) {
		...
		CanteenOrder order = new CanteenOrder(student, stall, totalPrice, request.pickupSlot(), queueNumber);
	...
	private void assertNoActiveOrder(UserAccount student, java.util.UUID stallId) {
		if (orderRepository.existsByStudent_IdAndStall_IdAndStatusIn(student.getId(), stallId, ACTIVE_STATUSES)) {
	...
	private int nextQueueNumber(java.util.UUID stallId, LocalDateTime pickupSlot, int queueLimit) {
		...
		long activeOrders = orderRepository.countByStall_IdAndPickupSlotBetweenAndStatusIn(stallId, start, end, ACTIVE_STATUSES);
